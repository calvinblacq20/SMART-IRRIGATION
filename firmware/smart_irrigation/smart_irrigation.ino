...
------------------------------------------------------------------------------*/
int SOIL_RAW_DRY   = 3200;   // reading in dry air / bone-dry soil
int SOIL_RAW_WET   = 1300;   // reading in a glass of water
int LDR_RAW_DARK   = 200;    // reading in the dark
int LDR_RAW_BRIGHT = 3500;   // reading in bright light
...
struct Settings {
  ...
  uint16_t pumpMaxRun   = 30;   // s  safety cut-off for one watering
  uint16_t pumpCooldown = 60;   // s  minimum wait between waterings
...
void setup() {
  ...
  // Wi-Fi + HTTP API (smart_irrigation_wifi.ino)
  wifiSetup();
}
